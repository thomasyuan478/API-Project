...
router.post('/:eventId/attendance', requireAuth, async (req,res,next) => {
  ...
  if(event.Group === null){
    const err = new Error("Forbidden: Current User must be a member of the group to request attendance");
    err.status = 403;
    return next(err);
  }
...
router.post('/:eventId/images', requireAuth, async (req,res,next) => {
  ...
  if(validAttendee || validMembership || event.Group.organizerId === req.user.id){
  console.log(true);
  ...
  const newImage = await EventImage.create({
    ...
    const err = new Error("Forbidden: Current user is not an attendee, host or organizer");
    err.status = 403;
    return next(err);
  }
});
...
const event = await Event.findByPk(req.params.eventId, {
  ...
  const err = new Error("Forbidden: Current User must be the organizer or the co-host");
  err.status = 403;
  return next(err);
});
...
router.put('/:eventId', requireAuth, async (req,res,next) => {
  ...
  if(venueCheck(venueId, venue) || nameCheck(name) || typeCheck(type) || integerCheck(capacity) || priceCheck(price) ||
    !description || startDateCheck(startDate) || endDateCheck(startDate, endDate)){
      ...
      err.status = 400;
      err.errors = {};
    if(venueCheck(venueId, venue)) err.errors.venueId = "Venue does not exist";
  ...
  res.json(
    ...
    const err = new Error("Forbidden: Current User must be the organizer or a co-host");
    err.status = 403;
    next(err);
  }
});
...
if(checkId(userId)){
  ...
  else if(typeof userId !== 'number') err.errors.userId = "MemberId must be a number";
  return next(err);
}
...
const err = new Error("Current User must be organizer or the same id as being deleted");
err.status = 403;
return next(err);
}
});
...
router.delete('/:eventId', requireAuth, async (req,res,next) => {
  ...
  if(validMembership || event.Group.organizerId === req.user.id){
    ...
    const err = new Error("Forbidden: Current User must be the owner or a co-host");
    err.status = 403;
    return next(err);
  }
});
