...
router.get("/", async (req, res) => {
  const groups = await Group.findAll({
    include: [{ model: Membership }, { model: GroupImage }, { model: Event }],
  });
...
router.get("/current", requireAuth, async (req, res) => {
  ...
  if (groups.length === 0 && membergroups.length === 0)
    return res.json({ message: "User is not part of any groups." });
...
router.get("/:groupId/venues", requireAuth, async (req, res, next) => {
  ...
  if (valid || groups.organizerId === req.user.id) {
    ...
    const err = new Error(
      "Forbidden: Must be group owner or co-host to search for venues."
    );
    err.status = 403;
    return next(err);
  }
...
router.get("/:id", async (req, res, next) => {
  const group = await Group.findByPk(req.params.id, {
    include: [{ model: GroupImage }, { model: User }, { model: Venue }],
  });
...
router.post("/:groupId/venues", requireAuth, async (req, res, next) => {
  ...
  if (valid || group.organizerId === req.user.id) {
    ...
    const err = new Error(
      "Forbidden: Must be group owner or co-host to create new venues."
    );
    err.status = 403;
    return next(err);
  }
...
router.post("/:groupId/membership", requireAuth, async (req, res, next) => {
  ...
  if (
    pendingMembership.status === "member" ||
    pendingMembership.status === "co-host" ||
    pendingMembership.status === "organizer"
  ) {
...
router.put("/:groupId/membership", requireAuth, async (req, res, next) => {
  ...
  if (
    status === "member" &&
    (req.user.id === group.organizerId || currentUser.status === "co-host")
  ) {
    ...
    res.json(resUpdate);
  } else if (status === "co-host" && req.user.id == group.organizerId) {
    update.status = status;
    await update.save();
...
router.delete("/:groupId/membership", requireAuth, async (req, res, next) => {
  ...
  if (checkId(memberId)) {
    ...
    if (!memberId) err.errors.memberId = "Invalid memberId input";
      ...
      err.errors.memberId = "MemberId must be a number";
    return next(err);
  }
  ...
  const err = new Error(
    "Forbidden: Current User must be host or the owner of the membership being deleted"
  );
  err.status = 403;
  return next(err);
});
